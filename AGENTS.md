...
3. Agents never start servers or long-running processes (no `npm run dev`, `astro preview`, `wrangler dev`), and never deploy on their own: Mladen checks the build locally first and gives the word for `wrangler deploy`. Mladen starts them in his terminal and says which URL is up; agents then verify against that URL with the browser tools or curl. Agents verify what they can without a server: `npm run build`, static checks of `dist/`, unit-level scripts.
...
- Releases: every deploy is tagged with semantic versioning, `vMAJOR.MINOR.PATCH`, as an annotated tag on the deployed commit, pushed with `main`. Major = a new site generation (v1 Jekyll, v2 Astro on Cloudflare), minor = a batch of features or a milestone, patch = a fix-only deploy. The tag message names what shipped. `git tag -l 'v*'` is the deploy log.
