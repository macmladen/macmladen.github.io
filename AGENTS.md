...
- Task lifecycle: the first step of any task is `backlog task edit <n> -s "In Progress"`; the last step is administration (tick acceptance criteria, notes with effort figures, final summary) and `-s Done`. Never work on a task that is not In Progress.
