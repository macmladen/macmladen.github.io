...
|---|---|---|
| name | text | no |
| city | text, placeholder "Novi Sad", `autocomplete="address-level2"`, up to 100 characters; asked of watchers too, so it carries no `data-laptop-only` | no |
| email | email | yes |
...
D1 table `registrations` additionally has `id`, `created_at`, `ip_hash`, `mailerlite_status` and `confirmation_status`. `terminal` arrived after the first deploy, in `migrations/0003_registrations_terminal.sql`, `confirmation_status` in `migrations/0004_registrations_confirmation.sql` and `city` in `migrations/0005_registrations_city.sql`; all three are therefore nullable, because the rows written before them carry no answer and a default would invent one.
...
- Sharing cards, one per page, 1200×630: sand ground, ink title set large, an optional smaller line under it in the softer ink, the `h1`'s accent rule at six pixels, and the name and site URL along the bottom. `scripts/og.mjs` draws them with satori and sharp into `public/og/<slug>.png` before every build (`npm run og`, wired as npm's `prebuild`); the directory is generated and git-ignored. `src/data/og.ts` is the registry both ends read — the script to know what to draw, `Head.astro` to know what to name, looked up by canonical path. Card titles come from the same data objects the pages use; the home card carries the standfirst rather than repeating the name its footer line already shows. The workshop card is the only one with the second line, and it answers "when and where" before anyone opens the link: `whenAndWhere()` in `src/lib/dates.ts` composes it from `workshop.start` and `workshop.venue` in Europe/Belgrade, and `/speaking/` builds the same line for its highlighted entry out of the same helpers — the card as one string, the page with the date and time inside a `<time>` and the venue beside it. Inter (latin, 400 and 700, SIL OFL) is checked into `src/assets/fonts/` because satori needs font data and the site itself loads no fonts. Not an Astro endpoint: `@astrojs/cloudflare` prerenders inside workerd, where a route can neither read a font off disk nor call an image library.
- `rel="me"` on the LinkedIn, GitHub and Speaker Deck links in the footer, so the profiles verify back to the site.
- Every link that leaves the site opens in a new tab and says so: `target="_blank"`, `rel="noopener"` (added to whatever `rel` the link already carries, `me` included), and a small arrow after the text drawn by `a[target="_blank"]::after` in `src/styles/components.css` — Material Design's "open_in_new" on a 24px grid, inlined as a mask so it takes `currentColor`, sized at `0.8em`. The `.button`s and the footer's profile marks are excluded from the arrow: one is already a destination, the others have no visible text for an arrow to follow. Outward means an `href` beginning `http://` or `https://` whose host is neither `macmladen.com` nor `www.macmladen.com`; relative links, fragments and `mailto:` are left alone, and `data-no-external` on an anchor opts it out entirely. The attributes are added by `scripts/external-links.mjs`, which rewrites the built pages in `dist/client` as npm's `postbuild` hook, so the components stay readable — with one exception: what the two endpoints render at runtime is never a file that step can reach, so `Footer.astro`, `RegistrationClosed.astro` and `RegistrationSuccess.astro` carry the attributes in their own source, and the script leaves alone anything that already has them.
...
## Acceptance criteria
- [ ] `npm run dev` serves the five pages locally (no DDEV: removed 2026-09-08).
- [ ] `/` shows intro, workshop announcement and footer in order; the announcement carries the classroom image with the credit strip "Image: AI-generated (OpenAI)" (MM-23), eyebrow, title, intro and CTA to the workshop page; every footer link resolves to the stated URL.
...
- [ ] The workshop page shows the corrected EN abstract, the mandatory checklist, the prerequisites paragraph linking the real repository, the close date, and the form with its ten controls (email required, the rest optional, MM-73); the JSON-LD validates as an `Event`.
...
- [ ] With JavaScript disabled both forms validate and re-render errors, but cannot complete: Turnstile mints its token in the browser, so the `<noscript>` block on each form points to email instead (decision recorded 2026-09-10). With JavaScript, both forms submit in place (MM-68). `/`, `/about/` and `/speaking/` ship no script at all; the workshop page ships Turnstile and one inline script of its own (the copy buttons, and the registration form submitted with `fetch`); `/contact/` ships Turnstile and one inline script of its own (the topic preselect, and the contact form submitted with `fetch`). Those two are the only inline scripts on the site and each is in the CSP by hash. No external JavaScript anywhere except Turnstile's.
...
- [ ] With the close date in the past, a rebuilt page shows the closed notice instead of the form; a page built before the close date gets the notice from the endpoint's 403 response, which the page script swaps in (2026-09-10). Rebuild and deploy on the day after the close date anyway.
...
- [ ] Lighthouse mobile on all five live pages: performance, best practices and SEO 100; accessibility 95 or better. The remaining accessibility points are the #F40 link and button text (3.05:1 and 3.45:1), Mladen's decision of 2026-09-09; the two criteria are reconciled here. Verified 2026-09-10: 100/95–96/100/100 (`docs/verification-2026-09-10.md`).
- [ ] README documents local run, deploy, D1 create/bind/migrate, secrets, CSV export, DNS change.
- [ ] Commits per task as AGENTS.md prescribes; `git status` clean; no `.dev.vars` in the tree; no token values anywhere.
