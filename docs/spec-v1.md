...
- "Registration closes Thursday, 17 September 2026." The band carries `id="register"`, which is what every "Register for the workshop" link on the site points at (MM-62). The close date is written once, as `closesAt` in `src/data/workshop.ts`; this line, `isRegistrationOpen`, the Event offer's `validThrough` and the closed-state line all derive from it, and the visible weekday is formatted in Europe/Belgrade by `src/lib/dates.ts` (MM-62). The form, fields below. Success state replaces the form inline with a thank-you saying a confirmation email is on its way. After the close date the form is replaced by a "Registration is closed" line; the page stays as the talk's archive page for slides and recording later.
